  ...
  const SampaolesiChart = () => {
    ...
    const addPoint = () => {
      ...
      if (!isNaN(x) && !isNaN(y) && x >= minX && x <= maxX && y >= minY && y <= maxY) {
        setPoints([...points, { x, y, eye: selectedEye, id: `${x}-${y}-${selectedEye}` }]);
        setSelectedX('');
        setSelectedY('');
      } else {
            ...
            <div className="flex justify-center">
              <svg width={width} height={height} className="border border-chart-border bg-chart-bg rounded-lg">
                <g transform={`translate(${margin.left}, ${margin.top})`}>
                  ...
                  {/* Axis titles */}
                  {/* Hover guide lines and axis labels */}
                  {hoveredPoint && (
                    <>
                      {/* Linha vertical (guia do eixo X) */}
                      <line
                        ...
                        strokeWidth="1"
                      />
                      {/* Linha horizontal (guia do eixo Y) */}
                      <line
                        x1={0}
                      ...
                      {/* Valor no eixo X */}
                      <text
                        x={scaleX(hoveredPoint.x)}
                        y={chartHeight - 8}
                        textAnchor="middle"
                        fontSize="12"
                        className="fill-chart-text font-semibold"
                      >
                        {hoveredPoint.x}
                      </text>
                  ...
                  return (
                    <g key={point.id}>
                      <circle
                        cx={scaleX(point.x)}
                        cy={scaleY(point.y)}
                        r={isHovered ? 8 : 6} // Aumenta o raio via JS
                        fill="currentColor"
                        stroke="white"
                        strokeWidth="2"
